...
@component.add(
    name="Result", comp_type="Auxiliary", comp_subtype="Normal", depends_on={"x": 3}
)
def result():
    return integer(x()) + if_then_else(x() > integer(x()), lambda: 1, lambda: 0)


@component.add(name="X", comp_type="Constant", comp_subtype="Normal")
def x():
    return 2.1
